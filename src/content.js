...
import { createTaskDom, deleteAllTasksDom } from "./task/task-dom.js";
...
function createContent() {
  ...
  addTaskButton.addEventListener("click", () => {
    ...
    const dialogDeleteBtn = dialog.querySelector(
      ".dialogForm #taskButtonsDiv #deleteTaskBtn",
    );
...
export function loadProjectIntoContent(projectId) {
  ...
  const ago = formatDistance(projects[projectId].dueDate, new Date(), {
    addSuffix: true,
  });
  ...
  detailsDiv.addEventListener("click", () => {
    ...
    priorityOptionsArr.forEach((option) => {
      if (option.value === projectPriority) {
        option.checked = true;
      } else {
