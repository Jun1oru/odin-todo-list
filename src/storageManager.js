...
import { createProjectDebug } from "./sidebar/projects";
...
export function loadProjects() {
    if(!localStorage.getItem("projects")) {
        ...
        let projectsArr = Array.from(JSON.parse(localStorage.getItem("projects")));
        projectsArr.forEach((project) => {
            if(project === null || project == undefined) { return; }
            createProjectDebug(project.title, project.description, project.priority, project.dueDate);
            project.todos.forEach((todo) => {
                if(todo === null || todo === undefined) { return; }
                createTaskDebug(todo.project, todo.title, todo.description, todo.priority, todo.dueDate, todo.done);
            });
        });
